...
describe('Test promise concatenation', () => {
    ...
    it('should switch to/from callback functions combine', () => Promise.fCombine({
        step1: (init, done) => done(null, 'step1'),
        step2: (step1Result, done) => done(null, 'step2')
    }, { init: 'init' })._combine({
        step3: () => Promise.resolve('step3')
    })._fCombine({
    ...
    it('concat object remix!', () => Promise.aggregate({
    ...
    it('concat array remix!', () => Promise.merge([
    ...
    it('execute aside task.', () => Promise.resolve('Original')
        ._aside(stream => Promise.resolve(`Aside from ${stream}`))
        .then(stream => stream.should.equal('Original'))
    );

    it('execute aside task no promise.', () => Promise.resolve('Original')
        ._aside(stream => `Aside from ${stream}`)
        .then(stream => stream.should.equal('Original'))
    );

    it('execute aside task with breaking error.', () => Promise.resolve('Original')
        ._aside(stream => Promise.reject(`Aside from ${stream}`))
        .catch(error => error.should.equal('Aside from Original'))
    );

    it('execute aside task ignoring error.', () => Promise.resolve('Original')
